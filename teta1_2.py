...
def nadji_teta1_teta2(teta, d_c, r_l, r_o, g, H):
    # uslov (8)
    if not atan(H/d_c) < teta < prebaci_u_radijan(90):
        return teta, teta

    v = nadji_brzinu(d_c, teta, H, g)
    min_teta = max(asin(sqrt(2*g*H/v**2)), atan(H/d_c)) + prebaci_u_radijan(0.1)
...
if __name__ == "__main__":
    d_c = 4.57-0.15-0.23  # daljina do centra obruca
